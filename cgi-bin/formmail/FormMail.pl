...
$mailprog = '/usr/sbin/email';        

# @referers allows forms to be located only on servers which are defined     #
# in this field.  This security fix from the last version which allowed      #
# anyone on any server to use your FormMail script on their web site.        #

@referers = ('naorp.org','216.251.32.92', '216.235.70.27');
...
sub send_mail {
    ...
    local($print_config,$key,$sort_order,$sorted_field,$env_report);
		#system('echo "labas" | mail -s "Subject" razmas@gmail.com -aFrom:webmaster@physemp.com');
		#print STDERR Dumper('echo "labas" | mail -s "'.$Config{'subject'} ? $Config{'subject'} :'WWW Form Submission'.'" '.$Config{'recipient'}.' -aFrom:'.$Config{'email'}.' ('.$Config{'realname'}.')')
    my $str = "Below is the result of your feedback form.  It was submitted by\n";
    $str.= "$Config{'realname'} ($Config{'email'}) on $date\n";
    $str.= "-" x 75 . "\n\n";

    if (@Print_Config) {
        foreach $print_config (@Print_Config) {
            if ($Config{$print_config}) {
                $str.= "$print_config: $Config{$print_config}\n\n";
            }
        }
    }
    ...
    if ($Config{'sort'} =~ /^order:.*,.*/) {
        ...
        foreach $sorted_field (@sorted_fields) {
            if ($Config{'print_blank_fields'} || $Form{$sorted_field} ne '') {
                $str.= "$sorted_field: $Form{$sorted_field}\n\n";
            }
        }
    }
    ...
    foreach $env_report (@Env_Report) {
        if ($ENV{$env_report}) {
            $str .= "$env_report: $ENV{$env_report}\n";
        }
    }

		system('echo "'.$str.'" | mail -s "'.($Config{'subject'} ? $Config{'subject'} :'WWW Form Submission').'" '.$Config{'recipient'}.' -aFrom:"webmaster@physemp.com ('.$Config{'realname'}.')"');
}
...
sub fill_recipients {
    ...
    foreach $domain (@domains) {
		...
		push(@return_recips,"taterrien\@gmail.com");

#		push(@return_recips,"razmas\@gmail.com");
